...
object MostPopularSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    val mostTenPopular = flipped.take(10)
    ...
    val mostTenPopularName = mostTenPopular.map(x => (namesRdd.lookup(x._2).head,x._1))

    // Print out our answer!
    println(s"$mostPopularName is the most popular superhero with ${mostPopular._1} co-appearances.")

    for(row <- mostTenPopularName){
      println(s"${row._1} with ${row._2} co-appearances")
    }
  }
